...
void handleconfig() {
    ...
    //网页界面代码段
    String content = "<html><meta charset='UTF-8'><style>html,body{ background: #1aceff; color: #fff; font-size: 10px;}</style>";
    content += "<body><form action='/' method='POST'><br><div>天气时钟</div><br>";
    content += "城市代码:<br><input type='text' name='web_ccode' placeholder='city code'><br>";
    content += "<br>时钟亮度(1-100):(推荐:15)<br><input type='text' name='web_bl' placeholder='10'><br>";
    content += "<br>天气更新分钟:(默认:10分钟)<br><input type='text' name='web_upwe_t' placeholder='10'><br>";
#if DHT_EN
    content += "<br>室内温湿度  <input type='radio' name='web_DHT11_en' value='0'checked> 关 \
