...
void Display::TFT_init() {
    ...
    //屏幕方向  1USB朝右  2USB朝上  3USB朝左  4USB朝下
...
void Display::loading(byte loadNum) {
    ...
    clk.setTextColor(TFT_BLACK);    //设置文本颜色--文本颜色,背景颜色(可删除)
    //绘制字符串--字符串,x,y
    clk.drawString("Connecting to WiFi...", 98, 20);
    //实心圆角矩形--x,y,长度,宽度,圆角半径,颜色
    clk.fillRoundRect(0, 35, 200, 6, 3, 0x7BEF);
    //实心圆角矩形--x,y,长度,宽度,圆角半径,颜色
    clk.fillRoundRect(0, 35, loadNum, 6, 3, TFT_ORANGE);
    clk.pushSprite(20, 160);         //窗口位置--x,y
    clk.deleteSprite();             //删除窗口
}
...
/* 清屏 */
//绘制颜色
void Display::TFT_CLS(int bgColor) {
    tft.fillScreen(bgColor);        //清屏
}

/* app1 */
void Display::app1(unsigned char Get_Hour, unsigned char Get_Minute, unsigned char Get_Second, unsigned char Get_Month,
                   unsigned char Get_Day, unsigned char Get_Week) {
    if (APPflag == true) {
        tft.fillScreen(TFT_WHITE);        //清屏
    }
    //时钟刷新
    if (Get_Hour != Hour_sign || APPflag == true) {
        // dig.printfB80120(10, 2, Get_Hour / 10);
        // dig.printfB80120(90, 2, Get_Hour % 10);
    ...
    if (Get_Second % 2 == 0){
        //图片--x,y,图片  
        TJpgDec.drawJpg(180, 2, O_40120_colon, sizeof(O_40120_colon)); 
    } else {
    ...
    //分钟刷新
    if (Get_Minute != Minute_sign || APPflag == true) {
        dig.printfO80120(70, 120, Get_Minute / 10);
        dig.printfO80120(150, 120, Get_Minute % 10);
        ...
        clk.fillSprite(TFT_WHITE);      //窗口填充--颜色
        clk.setTextDatum(CC_DATUM);     //设置文本数据
        //设置文本颜色--文本颜色,背景颜色(可删除)
        clk.setTextColor(TFT_BLACK, 0x263D);
        //实心圆角矩形--x,y,长度,宽度,圆角半径,颜色
        clk.fillRoundRect(15, 17, 54, 24, 5, 0x263D);
        //绘制字符串--字符串,x,y
        clk.drawString(MONTH, 42, 30);
        //实心圆角矩形--x,y,长度,宽度,圆角半径,颜色
        clk.fillRoundRect(15, 47, 54, 24, 5, 0x263D);
        //绘制字符串--字符串,x,y
        clk.drawString(DAY, 42, 60);
        //设置文本颜色--文本颜色,背景颜色(可删除)
        clk.setTextColor(TFT_BLACK, 0xEC44);
        //实心圆角矩形--x,y,长度,宽度,圆角半径,颜色
        clk.fillRoundRect(15, 77, 54, 24, 5, 0xEC44);
...
String cityCode = "101280601";    //城市代码
...
void getCityWeater() {
    ...
    String URL = "http://d1.weather.com.cn/weather_index/" + cityCode + ".html?_=" + String(millis());//原来
...
void weaterData(String *cityDZ, String *dataSK, String *dataFC) {
    ...
    clk.setTextDatum(CC_DATUM);       //设置文本数据
    //实心圆角矩形
    clk.fillRoundRect(0, 0, 60, 26, 4, pm25BgColor);
    clk.setTextColor(TFT_BLACK);      //设置文本颜色--文本颜色,背景颜色(可删除)
    ...
    clk.setTextDatum(CC_DATUM);       //设置文本数据
    //实心圆角矩形
    clk.fillRoundRect(0, 0, 115, 26, 4, pm25BgColor);
    clk.setTextColor(TFT_BLACK);      //设置文本颜色--文本颜色,背景颜色(可删除)
...
void scrollBanner() {
    ...
    clk.setTextColor(TFT_BLACK);      //设置文本颜色--文本颜色,背景颜色(可删除)
    //绘制字符串--字符串,x,y
    clk.drawString(scrollText[currentIndex], 90, 13);
...
void Display::app2(int xms) {
    ...
    //*分钟更新一次天气
    if (millis() - WeaterTime1 > UpdateWeater_Time * 60000 || APPflag == true) {
        WeaterTime1 = millis();
